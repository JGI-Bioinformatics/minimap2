...
{
	const char *opt_str = "2aSDw:k:K:t:r:f:Vv:g:G:I:d:PXT:s:x:Hcp:M:n:z:A:B:O:E:m:N:Qu:R:hF:LC:yY";
	mm_mapopt_t opt;
	mm_idxopt_t ipt;
	...
	while ((c = getopt_long(argc, argv, opt_str, long_options, &long_idx)) >= 0) {
		...
		else if (c == 0 && long_idx ==33) opt.flag |= MM_F_OUT_MINS; // --output-min
