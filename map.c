...
fprintf(stderr, "%d minimizers. regs0=%p n_regs0=%d\n", n_mini_pos, regs0, n_regs0);
		if ((opt->flag & MM_F_OUT_MINS) && regs0 && regs0->p) {
			// record ref and query minimizer positions for later output
			mm_minipos_v *minipos = &(regs0->p->minipos);
			kv_resize(mm_minipos_t, 0, *minipos, n_mini_pos);
			int32_t pos;
			for(pos=0; pos < n_mini_pos; pos++) {
				uint32_t qpos = (uint32_t)(a[pos].y&0xffffffff)>>1;
				mm_minipos_t tmp = { qpos, (uint32_t)(mini_pos[pos]&0xffffffff) };
